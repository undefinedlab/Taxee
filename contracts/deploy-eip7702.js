...
async function main() {
  ...
  // Also save to eip7702 folder
  const eip7702DeploymentPath = path.join(__dirname, "eip7702", "deployments", `${deploymentInfo.chainId}.json`);
  const eip7702DeploymentsDir = path.join(__dirname, "eip7702", "deployments");
  if (!fs.existsSync(eip7702DeploymentsDir)) {
    fs.mkdirSync(eip7702DeploymentsDir, { recursive: true });
  }
  fs.writeFileSync(eip7702DeploymentPath, JSON.stringify(deploymentInfo, null, 2));
  console.log("💾 Also saved to:", eip7702DeploymentPath);
